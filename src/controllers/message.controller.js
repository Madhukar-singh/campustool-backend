...
exports.newThread = async (req, res, next) => {
  try {
    ...
    const first_participant =
      req.user._id > req.body.to ? req.body.to : req.user._id;
    const second_participant =
      req.user._id > req.body.to ? req.user._id : req.body.to;
...
exports.getInbox = async (req, res, next) => {
  try {
    ...
    const threadList = await sql.query(
      `SELECT * FROM conversation WHERE user_id = '${req.user._id}'`
    );
    for (let index = 0; index < threadList.length; index++) {
      ...
      const receiverId =
        thread.first_participant + "" === req.user._id + ""
          ? thread.second_participant
          : thread.first_participant;
      const receiverApplicant = await Applicant.findOne({
        id: receiverId,
      }).exec();
      const receiverRecruiter = await Recruiter.findOne({
        id: receiverId,
      }).exec();
      const receiver =
        receiverApplicant === null ? receiverRecruiter : receiverApplicant;
      const threadData = {
        thread,
